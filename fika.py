...
class WebSocketHandler(tornado.websocket.WebSocketHandler):
    ...
    def buttonA(self, name):
        if self.requested_fika==None:
            print name, "requested fika"
            self.requested_fika=name
            self.confirmed_fika[name]=True
            for mb in connected_microbits:
                if mb!=name:
                    self.bt.putLed(mb, 'Fika?')
        else:
            print name, "confirmed fika"
            self.confirmed_fika[name]=True

    def buttonB(self, name):
        if name==self.requested_fika:
            for mb in self.confirmed_fika:
                print "Called", mb, "to fika"
                self.bt.putLed(mb, 'Fika!')
        self.reset_fika()
...
class Bluetooth():
    ...
    def putLed(self, name, msg):
        self.led_iface[name].WriteValue(msg, ())
        print "Sent", msg
